...
class FacialDetectionTest extends TestCase
{
    ...
    public function test_a_user_cannot_use_an_image_with_no_face()
    {
        ...
        ParkwayFaceTechService::driver(AwsFacialAdaptor::class)->facialDetectionService()->detectFileImage(new File(base_path('Images/7.jpeg')));
    }

    public function test_face_detection_works_with_image_with_face()
    {
        $response = ParkwayFaceTechService::driver(AwsFacialAdaptor::class)->facialDetectionService()->detectFileImage(new File(base_path('Images/1.png')));
